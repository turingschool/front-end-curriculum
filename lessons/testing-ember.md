...
FOLLOW UP:
- Testing store in integration test
- Run through order of operations
- Diff between Integration/Acceptance
- How to clean test suite before each run
- Other types of Unit testing?
- **Note issue with Ember.Object thing for get method.**
...
Our `moduleFor` test-helper provide us with the method `this.subject` that handles figuring out what model or object we are talking about and instantiating an instance of it.  

Unit tests are where any Object methods would be tested as well. For instance, if your model looked something like this:
...
```
As a user, when I visit the home page, and I enter values for Name, Quantity, Notes, and hit the Submit button, I see a Grocery item with those values.
```
...
`// tests/integration/components/add-grocer-test.js`

```
test('it starts with default properties', function(assert))
```

[Component Testing Docs](https://guides.emberjs.com/v2.8.0/testing/testing-components/)
...
We will be using the [ember-groceries](https://github.com/turingschool-examples/ember-groceries) repository.  

You will probably also need [the docs](https://guides.emberjs.com/v2.8.0/testing/).

Replace `skip` with `test` in your test files and fill in the content in the following files:  
`test/acceptance/add-grocery.js`
`test/unit/models/grocery.js`
`test/unit/controllers/groceries-test.js`
